...
class EncounterSpell:
    ...
    def to_string(self):
        return f'name:{self.spell_name} id:{self.spell_id} encounter:{self.journal_encouter_id} sections:{self.journal_section_ids}'

#------------------------------------------------------------------------------
class InstanceVoicePack:

    def __init__(self):
        self.instance_name = ""
        self.lang = ""
        self.isDungeons = True
        ...
        self.encounter_id_to_spells = {}

    def to_string(self):
        return f'expansion:{self.expansion_name}-{self.journal_expansion_id} name:{self.instance_name} isDungeons:{self.isDungeons} jinstance:{self.journal_instance_id} instance:{self.instance_id}'
...
#------------------------------------------------------------------------------
def create_voice_packs_from_instance_id(journal_instance_id):

    vp = InstanceVoicePack()

    journalInstance = get_response('journal-instance', journal_instance_id).json()
     
    vp.journal_instance_id = journal_instance_id
    vp.instance_id = journalInstance["map"]["id"]
    vp.instance_name = journalInstance["name"]

    for encounter in journalInstance["encounters"]:
        journal_encounter_id = encounter["id"]
        checkJournalEncounter(journal_encounter_id)
        

    # for instance in journalExpansion["dungeons"]:
    #     vpdb = vpdb + create_voice_packs_from_instance_id(instance["id"])


    # for instance in journalExpansion["raids"]:
    #     vpdb = vpdb + create_voice_packs_from_instance_id(instance["id"])

    # name = journalExpansion["name"]
    # for vp in vpdb:
    #     vp.expansion_name = name
    #     vp.journal_expansion_id = journal_expansion_id

    return vp


#------------------------------------------------------------------------------
def create_voice_packs_from_expansion_id(journal_expansion_id):
    ...
    for instance in journalExpansion["dungeons"]:
        vp = create_voice_packs_from_instance_id(instance["id"])
        vp.isDungeons = True
        vpdb.append(vp)
...
#------------------------------------------------------------------------------
def create_voice_packs_from_expansion_ids(expansion_ids):
    vpdb = []
    for expansion_id in expansion_ids:
        voice_packs = create_voice_packs_from_expansion_id(expansion_id)
        vpdb.extend(voice_packs)
    return vpdb
...
vpdb = create_voice_packs_from_expansion_ids([503])
for vp in vpdb:
    print(vp.to_string())
